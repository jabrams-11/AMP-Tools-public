...
static constexpr double LOOKAHEAD_STEP = 1;
...
// Helper function to calculate the angle bisector of two adjacent edges forming a corner
Eigen::Vector2d MyBugAlgorithm::calculateCornerBisector(const Eigen::Vector2d& vertex, 
                                                        const Eigen::Vector2d& prev_vertex, 
                                                        const Eigen::Vector2d& next_vertex) {
    // Calculate the two edge vectors
    Eigen::Vector2d edge1 = vertex - prev_vertex;  // Edge from prev_vertex to vertex
    Eigen::Vector2d edge2 = next_vertex - vertex;  // Edge from vertex to next_vertex
    
    // Normalize the edge vectors
    edge1.normalize();
    edge2.normalize();
    
    // Calculate the bisector as the average of the two edge directions
    // This gives us the direction that bisects the angle between the two edges
    Eigen::Vector2d bisector = edge1 + edge2;
    
    // Normalize the bisector
    bisector.normalize();
    
    // For outward-facing bisector (away from the obstacle), we need to ensure
    // it points away from the obstacle interior. Since we're working with
    // clockwise vertices, the bisector should point outward.
    // We can verify this by checking the cross product to ensure proper orientation
    double cross_product = edge1.x() * edge2.y() - edge1.y() * edge2.x();
    
    // If the cross product is positive, the angle is convex (pointing outward)
    // If negative, the angle is concave (pointing inward) - flip the bisector
    if (cross_product < 0) {
        bisector = -bisector;
    }
...
amp::Path2D MyBugAlgorithm::plan(const amp::Problem2D& problem) {
    ...
    while (((current_pos - problem.q_goal).norm() > GLOBAL_STEP_SIZE) && step_count < max_steps) {
        ...
        // Check if we're close enough to the goal to end
        if ((current_pos - problem.q_goal).norm() <= GLOBAL_STEP_SIZE) {
            path.waypoints.push_back(problem.q_goal);
            LOG("Goal REACHEDD at (" << current_pos.x() << "," << current_pos.y() << ")");
            break;
        }
        ...
        if (collision_obstacle == -1) {
            ...
            Eigen::Vector2d circumnavigation_start = current_pos;
            bool circumnavigated = false;
            double min_dist_to_goal = (current_pos - problem.q_goal).norm();
            Eigen::Vector2d closest_point = current_pos;
            // Collision detected, need to find the boundary point and wall normal
            auto [boundary_point, wall_normal, closest_vertex_index] = findClosestBoundaryAndNormal(current_pos, problem, collision_obstacle);
           ...
           // std::cout << "Press Enter to continue..." << std::endl;
            ...
            while (wall_follow_steps < 200000) { // Safety limit for wall following
                ...
                Eigen::Vector2d right_pos = current_pos + GLOBAL_STEP_SIZE* LOOKAHEAD_STEP * right_dir;
               // std::cout << "Press Enter to continue..." << std::endl;
               // std::cin.get();
             ...
             //   std::cout << "Press Enter to continue..." << std::endl;
              //  std::cin.get();
                ...
                if (((current_pos - closest_point).norm() < GLOBAL_STEP_SIZE * 10) && circumnavigated) {
                    // LOG("Current pos - min dist to goal (" << (current_pos - closest_point).norm() << ")");
                    // LOG("Global step size * 10 (" << GLOBAL_STEP_SIZE * 10 << ")");
                    // std::cout << "Press Enter to condsfsdftinue..." << std::endl;
                    // std::cin.get();
                    break;
                }
                ...
                if (((current_pos - circumnavigation_start).norm() < GLOBAL_STEP_SIZE * 20) && wall_follow_steps > 25 && not circumnavigated) {
            ...
            //   if ((current_pos - circumnavigation_start).norm() < GLOBAL_STEP_SIZE * 5 && wall_follow_steps > 5) {
            //         LOG("Returned to starting point of circumnavigation - breaking out");
            //         LOG("Closest point (" << closest_point.x() << "," << closest_point.y() << ")");
            //         std::cout << "Press Enter to continue..." << std::endl;
            //         std::cin.get();
